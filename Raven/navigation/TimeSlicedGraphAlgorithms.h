...
enum { target_found, target_not_found, search_incomplete, target_founding_now };
...
public:
    ...
    Graph_SearchAStar_TS(const graph_type& G,
        int                source,
        int                target,
        Raven_Bot* owner) :Graph_SearchTimeSliced<Edge>(AStar),
...
int Graph_SearchAStar_TS<graph_type, heuristic>::CycleOnce()
{
    ...
    // 우선순위큐에서 하나 m_Heap[--m_isize]값을 클로즈노드에 넣는다.
    int NextClosestNode = m_pPQ->Pop();


    //put the node on the SPT

      // m_ShortestPathTree안에 가야할 노드 인덱스로 저장함 m_ShortestPathTree[0]번 =0번노드에 가라..
      // 이걸 중간에 반납하면되지 않을까 싶은데
    m_ShortestPathTree[NextClosestNode] = m_SearchFrontier[NextClosestNode];

    // 6개 정도 찾으면 중간에 던지는데 끝포인트는 그대로인 현상을 고치면 된다. 
    // 혹은 찾은값만 넘겨주고 계속 찾는걸 해줘


    if (check_mid_target == true)
    {
        int count = 0;
        for (int i = 0; i < 340; i++)
        {
            if (m_ShortestPathTree[i] > 0)
            {
                count += 1;
            }
        }
        if (count > 3)
        {
            save = target_founding_now;
            temp_target = m_iTarget;
            m_iTarget = NextClosestNode;
            void* pTrigger =
                m_Graph.GetNode(this->GetPathToTarget().back()).ExtraInfo();
            ...
            Dispatcher->DispatchMsg(SEND_MSG_IMMEDIATELY,
                ...
                Msg_PathReady,
                pTrigger);
            m_iTarget = temp_target;
            check_mid_target = false;
            //return target_found;
        }
    ...
    for (const Edge* pE = ConstEdgeItr.begin(); !ConstEdgeItr.end(); pE = ConstEdgeItr.next())
    {
        ...
        if (m_SearchFrontier[pE->To()] == NULL)
        {
            ...
            m_path.emplace_back(NextClosestNode);
