...
int Raven_PathPlanner::CycleOnce()const
{
    ...
    // 사이클 원스 -> 종류파악 -> 중간에 4개정도 찾으면 넘겨줌 -> 그 결과를 저장-> 


    // 현재사이클에서 찾고있는 결과를 result에 저장
    int result = m_pCurrentSearch->CycleOnce(); //m_pCurrentSearch->Times~~.
    ...
    else if (result == target_found)
    {
        ...
        //will just be NULL if no trigger)
          ...
          //항목 유형을 검색한 경우 경로의 최종 노드가 전달자 트리거를 나타낸다.
          //    따라서 메시지의 추가 정보 필드에서 트리거에 포인터를 전달할 가치가 있다. 
          //    (트리거가 없는 경우 포인터가 NULL일 뿐)
        void* pTrigger =
